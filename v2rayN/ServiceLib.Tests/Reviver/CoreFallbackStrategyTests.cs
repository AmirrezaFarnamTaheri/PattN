...
public class CoreFallbackStrategyTests
{
    ...
    [Test]
    public async Task Generate_ShouldUseEffectiveCoreResolver_WhenCoreTypeIsNull()
    {
        var profile = NewVless(nameof(ETransport.ws));
        profile.CoreType = null;
        var session = new RepairSession { Original = ProfileSnapshot.Capture(profile) };
        var strategy = new CoreFallbackStrategy(
        ...
        await results.Count.Should().BeEqualTo(1);
        await results[0].Mutations[0].From.Should().BeEqualTo(ECoreType.sing_box.ToString());
        await results[0].Profile.CoreType.Should().BeEqualTo(ECoreType.Xray);
    }
