...
public partial class DiscoveryManagementWindow : WindowBase<DiscoveryManagementViewModel>
{
    public DiscoveryManagementWindow()
    {
        ...
        this.WhenActivated(disposables =>
        {
            ...
            this.OneWayBind(ViewModel, vm => vm.Busy, v => v.busyProgress.IsVisible).DisposeWith(disposables);
            ...
            ViewModel.BrowseCatalogFileInteraction.RegisterHandler(async interaction =>
            {
                var path = await UI.OpenFileDialog(new FilePickerFileType("JSON")
                {
                    Patterns = ["*.json"],
                });
            ...
            ViewModel.BrowseCatalogUpdateFileInteraction.RegisterHandler(async interaction =>
            {
                var path = await UI.OpenFileDialog(new FilePickerFileType("JSON")
                {
                    Patterns = ["*.json"],
            ...
            ViewModel.SaveCatalogArchiveInteraction.RegisterHandler(async interaction =>
            {
                var path = await UI.SaveFileDialog("JSON");
                interaction.SetOutput(path);
            }).DisposeWith(disposables);

            ViewModel.BrowseCatalogArchiveInteraction.RegisterHandler(async interaction =>
            {
                var path = await UI.OpenFileDialog(new FilePickerFileType("JSON")
                {
                    Patterns = ["*.json"],
            ...
            ViewModel.SaveRemoteSourceBundleInteraction.RegisterHandler(async interaction =>
            {
                var path = await UI.SaveFileDialog("JSON");
                interaction.SetOutput(path);
            }).DisposeWith(disposables);

            ViewModel.BrowseRemoteSourceBundleInteraction.RegisterHandler(async interaction =>
            {
                var path = await UI.OpenFileDialog(new FilePickerFileType("JSON")
                {
                    Patterns = ["*.json"],
