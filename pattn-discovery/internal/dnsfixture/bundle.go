...
func LoadExchangeBundle(path string) (*BundleRuntime, error) {
	...
	for _, step := range bundle.Exchanges {
		...
		if fixture.CapturedAt != "" {
			...
			if err != nil {
				return nil, fmt.Errorf("%s: invalid capturedAt: %w", step.Fixture, err)
			}
			if runtime.validationTime.IsZero() || capturedAt.After(runtime.validationTime) {
				runtime.validationTime = capturedAt.UTC()
			}
		}
...
func (r *BundleRuntime) ValidationTime() time.Time {
	if r.validationTime.IsZero() {
		return time.Now().UTC()
	}
	return r.validationTime
}
...
func exchangeKey(
	...
	if parsed, err := netip.ParseAddr(strings.TrimSpace(address)); err == nil {
		normalizedAddress = parsed.Unmap().String()
	}
